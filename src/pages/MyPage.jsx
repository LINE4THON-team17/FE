...
export function MyPage() {
  ...
  const [profileImageUrl, setProfileImageUrl] = useState(""); // 항상 우리가 쓰는 URL
  ...
  /** 프로필 영역 클릭 → 파일 선택창 열기 (수정 모드일 때만) */
  const handleProfileClick = () => {
    if (!editMode || !fileInputRef.current) return;
    fileInputRef.current.click();
  };

  /** 파일 선택 시 미리보기 & state 업데이트 */
  const handleProfileFileChange = (e) => {
    ...
    reader.onloadend = () => {
      if (typeof reader.result === "string") {
        setProfileImageUrl(reader.result); // 미리보기
      }
    };
  ...
  useEffect(() => {
    (async () => {
      ...
      try {
        ...
        // 프로필 이미지가 없다면 ▷ 랜덤 기본 이미지 부여 + 서버에 저장
        if (!img) {
          const randomImg =
            PROFILE_DEFAULTS[
